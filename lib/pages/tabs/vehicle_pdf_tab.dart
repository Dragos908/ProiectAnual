...
class _VehiclePdfTabState extends State<VehiclePdfTab> {
  ...
  // ── Filtre ───────────────────────────────────────────────────────────────────
  Widget _buildFiltersCard(BuildContext context, List<Vehicle> all) {
    final clase = _uniqueClase(all);
    final subclase = _uniqueSubclase(all);

    // Resetăm dacă valoarea selectată nu mai există în lista curentă
      ...
      child: Padding(
        ...
        child: Column(
          children: [
                  ...
                  child: DropdownButtonFormField<VehicleStatus?>(
                    ...
                    items: [
                      ...
                      ...VehicleStatus.values.map(
                          ...
                          value: s,
                          child: Text(
                            AppLocalizations.of(context).translate(s.translationKey),
                          ),
                        ),
                      ),
                    ],
            ...
            // Rând 2: clasă + subclasă
            Row(
              children: [
                Expanded(
                  child: DropdownButtonFormField<String>(
                    value: _selectedClasa,
                    decoration: InputDecoration(
                      labelText: AppLocalizations.of(context).translate('class'),
                      isDense: true,
                      border: OutlineInputBorder(
                        borderRadius:
                        BorderRadius.circular(ApprovalTheme.radiusSmall),
                ...
                Expanded(
                  child: DropdownButtonFormField<String>(
                    value: _selectedSubclasa,
                    decoration: InputDecoration(
                      labelText:
                      AppLocalizations.of(context).translate('subclass'),
                      isDense: true,
                      border: OutlineInputBorder(
                        borderRadius:
                        BorderRadius.circular(ApprovalTheme.radiusSmall),
                    ...
                    items: subclase
                        ...
                        .toList(),
                    onChanged: (v) {
                      setState(() => _selectedSubclasa = v ?? 'Toate');
                      _clearSelection();
                    },
                  ),
                ),
