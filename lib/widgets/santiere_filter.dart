...
/// Holds the currently active filter state for the santiere list.
class SantiereFilter {
  final SantierStatus? status;
  final DateTimeRange? perioadaCreare;

  const SantiereFilter({
    this.status,
    this.perioadaCreare,
  });
  ...
  List<Santier> apply(List<Santier> all) {
    return all.where((s) {
      ...
      if (perioadaCreare != null) {
        ...
        if (s.createdAt.isAfter(
            perioadaCreare!.end.add(const Duration(days: 1)))) {
          return false;
        }
      }
      return true;
